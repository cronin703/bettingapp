...
async function settleDate(date: string) {
  ...
  for (const s of scores) {
    ...
    if (p.direction !== 'skip' && p.line !== null) {
      result = actual === p.line ? 'push' : p.direction === 'over'
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Settle yesterday's results and today's if games are done
    const [yday, tday] = await Promise.all([settleDate(yesterday), settleDate(today)]);

    // Re-run model for today with updated lines
    ...
    for (const game of games) {
      ...
      const [lines, hi, ai, sched] = await Promise.all([
        fetchLines(game.home_team, game.away_team, today),
        fetchInjuries(game.home_team, today),
        fetchInjuries(game.away_team, today),
        fetchScheduleContext(game.home_team, game.away_team, today),
      ]);
      const input: GameInput = {
        home_team: game.home_team, away_team: game.away_team, date: today,
        tipoff_time: game.tipoff_time, line: lines.spread, total: lines.total,
        opening_line: lines.opening_line, injuries: [...hi, ...ai],
        pace_differential: null, schedule_context: sched,
      };
